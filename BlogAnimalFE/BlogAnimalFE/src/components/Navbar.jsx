...
import { CustomizedMuiSwitches } from "./SwitchButton";
...
const Navbar = () => {
  ...
  const unhidden = () => {
    const profileMenu = document.querySelector("div.hidden");
  ...
  return (
    <>
      <div className="flex justify-between items-center px-10 py-3 border-b border-neutral-700">
        <div className="flex items-center">
          ...
          <Form onSubmit={submitSearch} method="post">
            <input
              ...
              className={`p-3 rounded-full border border-neutral-700 ${bg==="dark"?"bg-neutral-700 text-neutral-300":"bg-white text-black"}`}
              required
            />
          </Form>
        </div>
          ...
          {navbarData.map((data) => (
            <Link onClick={() => setCurrentURL(data.alt)} to={data.link}>
              {data.alt == currentURL ? (
                <data.icon
                  sx={{ fontSize: "50px" }}
                  className={`${bg=="dark"?"text-white":"text-black"} mr-5`}
                />
              ) : (
        ...
        <div className="flex items-center">
          <CustomizedMuiSwitches />
          {account !== undefined ? (
            <>
              <div onMouseEnter={unhidden} className="flex items-center">
                <img
                ...
                />
                <div className={`text-lg font-medium ${bg=="dark"?"text-white":"text-black"}`}>
                  {account?.username}
                </div>
              </div>
              <div
                onMouseLeave={hidden}
                id="profileMenu"
                className={`hidden border absolute mt-56 rounded-2xl ${bg=="dark"?"text-white bg-neutral-700 border-neutral-600":"text-black bg-white"} font-medium z-10 right-5`}
              >
            ...
            <div className="flex">
              <button
                ...
                className="bg-blue-800 text-white font-medium p-2 rounded-2xl mr-3"
              >
                Đăng Kí
              </button>
              <button
                onClick={() => navigate("SignIn")}
                className="bg-blue-800 text-white font-medium p-2 rounded-2xl"
              >
                Đăng Nhập
              </button>
            </div>
