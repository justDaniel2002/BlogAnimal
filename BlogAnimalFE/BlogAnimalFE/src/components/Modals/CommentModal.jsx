...
export const BlogCommentModal = ({ handleClose, Blog }) => {
  ...
  const submit = async () => {
    const comment = document.querySelector(`input#comment`).value;
    if (comment?.length > 0) {
      const result = await api.uploadBlogComment(
        comment,
        Blog.blogId,
        account.accountId
      );
  ...
  return (
    <>
      {Blog ? (
        <div
          className={`${
            bg == "dark" ? "bg-neutral-900 text-white" : "bg-white text-black"
          } h-4/5 overflow-y-scroll`}
        >
          {account ? (
            <div className="mb-5">
              <div className="font-medium mb-3 flex">
              ...
              <div className="flex justify-between pr-5">
                <input
                  ...
                  className={`p-2 rounded-full w-2/3 ${
                    bg == "dark" ? "bg-neutral-700" : "bg-white border-2"
                  }`}
                  placeholder={`bạn đang nghĩ gì ${account.username}`}
                />
                <button
...
export const TradeCommentModal = ({ handleClose, Post }) => {
  console.log("Post", Post);
  const account = useRecoilValue(accountAtom);
  ...
  const submit = async () => {
    const comment = document.querySelector(`input#comment`).value;
    if (comment?.length > 0) {
      const result = await api.uploadTradeComment({
        content: comment,
    ...
    <>
      {Post ? (
        <div className="bg-neutral-900 text-white max-h-screen overflow-y-scroll">
          ...
          <div>
            {Post.tradeComments.map((comment) => (
              <>
                <div className="flex mb-5">
                  <div>
                    <img
